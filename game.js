...
function update() {
    paddles.forEach(p => {
        ...
        if (p.id === 'cpu') p.y += (ball.y - (p.y + p.h/2)) * 0.1;
    ...
    if (ball.x > canvas.width) { score++; App.coins += 25; App.updateUI(); resetBall(); }
}
...
function draw() {
    ...
    // PALAS Y BOLAS CUSTOM
    ctx.fillStyle = App.equipped.paddle;
    paddles.forEach(p => { ctx.fillRect(p.x, p.y, p.w, p.h); ctx.strokeStyle="white"; ctx.strokeRect(p.x,p.y,p.w,p.h); });
    
    ctx.fillStyle = App.equipped.ball;
    ctx.beginPath(); ctx.arc(ball.x, ball.y, ball.r, 0, Math.PI*2); ctx.fill();
    ctx.font = "14px Orbitron"; ctx.fillStyle = "white";
    ctx.fillText(`❤️ ${lives} | SCORE: ${score}`, 20, 25);
}

function resetBall() { ball.x = 400; ball.y = 200; ball.dx *= -1; }
function stopGame(w) {
    gameRunning = false; document.getElementById('ui-layer').style.display = 'flex';
