...
function startGame(m) {
    ...
    paddles = [{ x: 10, y: 150, w: 15, h: 80, isPlayer: true }]; // Jugador
    paddles.push({ x: 775, y: 150, w: 15, h: 80, isPlayer: false }); // CPU 1

    if (mode.includes('2v2')) {
        paddles.push({ x: 50, y: 150, w: 15, h: 60, isPlayer: false, isAlly: true }); // Aliado
        paddles.push({ x: 735, y: 150, w: 15, h: 60, isPlayer: false }); // CPU 2
    }
...
function loop() {
    ...
    // FÍSICA MODO CAOS
    if (mode.includes('caos')) {
        ball.dy += Math.sin(Date.now()/200) * 0.5;
    }
    ...
    if (ball.x < 0) { 
        lives--; if (lives <= 0) stopGame("CPU"); else resetBall();
    }
    if (ball.x > canvas.width) {
        score++; App.coins += 50; App.updateUI(); App.save(); resetBall();
    }
